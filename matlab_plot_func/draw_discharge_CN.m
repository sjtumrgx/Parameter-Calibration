...
for i = 1:length(file_list)
    ...
    title_text = sprintf('%s放电曲线 (RMSE: %.4f V)', c_rates{i}, rmse);
    title(title_text, 'FontSize', title_font_size);
    
    % Add legend
    legend({'实验数据', '仿真数据'}, 'FontSize', legend_font_size, 'Location', 'northeast', 'Box', 'off');
    ...
    figname = sprintf('%s%s_%s_with_error_CN.png', output_path, filename, c_rates{i});
...
for i = 1:length(file_list)
    ...
    % Add legend entries
    legend_entries{2*i-1} = sprintf('实验 (%s)', c_rates{i});
    legend_entries{2*i} = sprintf('仿真 (%s)', c_rates{i});
end
...
% Save figure in high resolution
figname = sprintf('%s%s_multi_condition_compact_CN.png', output_path, filename);
...
% Save figure in high resolution
figname = sprintf('%s%s_multi_condition_with_zoom_CN.png', output_path, filename);
exportgraphics(fig_multi_compact,figname,'Resolution',300)
fprintf('带放大区域的图形已保存至 %s\n', output_path); 